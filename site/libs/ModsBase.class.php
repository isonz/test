...
{	
	...
	{
		$mark = isset(static::$_dbconfig) ? static::$_dbconfig : 'DEFAULT';
	...
	{
		self::_init();
		return DB::getRows(static::$_table, array(), $select, $order);
	}
	...
	static function getList(array $where,  $select = '*', $order='id DESC')
	{
		if(!$where) return false;
		self::_init();
